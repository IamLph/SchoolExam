...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			// 根据类型获取试题信息
			this.getInfo(option.course_id,option.type);
		...
		methods: {
			...
			async getInfo(courseId,type) {
				...
				if(this.type != 4){
					api.getQuestionByCourseIdAndType({
						data:{
							courseId:courseId,
							type:type
						},success:res=>{
					...
					this.getQuestionByCourseIdType4(courseId,type)
				}
			},
			// 获取综合练习题
			getQuestionByCourseIdType4(courseId,type){
				// console.log(courseId+":"+type)
				let num = this.QUESTIONNUM
				let that = this
				api.getQuestionByCourseIdType4({
					data:{
						courseId:courseId,
						num:num
					},success:res=>{
