		...
		<view :class="block===1? 'block' : 'none'">
			...
			<view id="top-box" class="cu-bar bg-white solid-bottom">
				...
				<view class="action">
					...
					<view class="cu-btn bg-green shadow" style="margin-right: 30upx;" data-target="modalCard" @tap="cut">斩</view>
					<!-- 提交按钮 -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.getWrongQuestionByUserId(userId);
		},
		methods: {
			// 斩断题目
			cut(){
				// console.log("斩断")
				let examId = this.subjectList[this.subjectIndex].id;
				let userId = uni.getStorageSync('user').userId;
				let newSubjectList = this.subjectList;
				// console.log(newSubjectList);
				// console.log(this.subjectIndex)
				newSubjectList = newSubjectList.splice(this.subjectIndex,1)
			...
			getWrongQuestionByUserId(userId){
				let that = this;
				api.getUserWrong({
					data:{
						userId:userId
